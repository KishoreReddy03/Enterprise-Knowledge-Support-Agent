...
class SourceCited(BaseModel):
    ...
    title: str
    url: str
    relevance: str = Field(default="", description="Why this source was relevant")
...
class TicketProcessResponse(BaseModel):
    ...
    guardrail_warnings: list[str] = Field(
        default_factory=list,
        description="Guardrail warnings triggered (prompt injection, PII, rate limit, hallucination)",
    )
    pii_detected: list[str] = Field(
        default_factory=list,
        description="Types of PII detected and masked in the input",
    )
