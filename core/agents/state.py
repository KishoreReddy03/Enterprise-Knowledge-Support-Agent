...
class ContradictionInfo(TypedDict, total=False):
    """
...
class TicketState(TypedDict, total=False):
    ...
    reranked_results: list[SourceResult]
    """
    Cross-source reranked results from all 4 sources combined.
    Ordered by cross-encoder relevance score against the query.
    This is the authoritative ranked list used by synthesis and drafting.
    """
    ...
    """Factual claims flagged as ungrounded during verification, to feed back to DraftingAgent."""
    ...
    llm_routing_decision: str
    """
    Explicit categorical routing decision emitted by quality gate LLM.
    One of: 'approved', 'revise', 'escalate'. Empty string means the LLM
    did not return a valid enum — threshold fallback is used instead.
    """

    llm_parse_failures: Annotated[list[str], operator.add]
    """
    Accumulating log of agents whose LLM JSON output failed to parse.
    Each entry is an agent name: 'sufficiency_check', 'quality_gate'.
    Non-empty means at least one routing decision used a safe default
