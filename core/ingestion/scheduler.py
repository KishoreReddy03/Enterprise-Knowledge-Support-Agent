...
class IngestionOrchestrator:
    ...
    def __init__(self) -> None:
        ...
        # Connection for staleness updates
        try:
            self._db_conn = psycopg2.connect(settings.NEON_DB_URL)
    ...
    def _get_collection_name(self, source: str) -> str:
        """
        Get collection/table name for a source.
    ...
    async def run_incremental_update(self) -> UpdateReport:
        ...
        try:
            ...
            for doc in recent_docs:
                ...
                # Log to knowledge_updates (would use Database client)
    ...
    async def _extract_affected_features(self, changelog_content: str) -> list[str]:
        """
        Extract affected features from changelog entry.
        
        Args:
            changelog_content: Changelog entry text.
            
        Returns:
            List of affected feature names (empty list - feature extraction disabled).
        """
        # Feature extraction via Claude disabled
        return []
    ...
    async def _mark_stale_chunks(self, features: list[str]) -> int:
        ...
        Searches for chunks mentioning affected features and marks them as stale
        in the database so they can be deprioritized during retrieval.
        ...
        try:
            if not self._db_conn:
                logger.warning("No database connection available for marking stale chunks")
                return 0
            ...
            for feature in features:
                try:
                    ...
                    if results:
                        ...
                        stale_count += rows_affected
                        
                        logger.info(
                            f"Marked {rows_affected} chunks as stale for feature: {feature}"
                        )
                        cursor.close()
                        
                except Exception as e:
                    logger.error(f"Error marking stale chunks for feature '{feature}': {e}")
                    if self._db_conn:
                        self._db_conn.rollback()
                    continue

        except Exception as e:
            logger.error(f"Error in _mark_stale_chunks: {e}")
    ...
    async def _log_knowledge_update(
        ...
        """
        Log a knowledge update to the Database.
        
        Note: This is a placeholder that logs locally. 
        Full database integration will be in core/memory/db_client.py.
    ...
    def get_ingestion_stats(self) -> dict[str, Any]:
        ...
        Returns:
            ...
            - last_ingestion_per_source: Not implemented (requires Database)
        ...
        for collection_name in collections:
            try:
                ...
                if self._retriever.collection_exists(collection_name):
                    # For now, report as 1 (exists) since we don't have row count query built
                    stats["total_chunks_per_collection"][collection_name] = 1
                    
                    # Estimate size (384 dims * 4 bytes + payload)
                    estimated_mb = (384 * 4 / 1024 / 1024) * 1.5  # Rough estimate
