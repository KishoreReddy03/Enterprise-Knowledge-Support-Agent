...
class StripeDataScraper:
    ...
    Handles web scraping of Stripe docs and changelog,
    ...
    async def scrape_stripe_docs(
        ...
        """
        Scrape Stripe documentation by parsing the official OpenAPI specification.
        
        Args:
            sections: Ignored. Maintained for backwards compatibility.
                      
        Returns:
            List of RawDocument objects, one per API endpoint.
        """
        documents: list[RawDocument] = []
        url = "https://raw.githubusercontent.com/stripe/openapi/master/openapi/spec3.json"
        logger.info(f"Fetching Stripe OpenAPI spec: {url}")

        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=self.STRIPE_TIMEOUT_SECONDS) as response:
                    if response.status != 200:
                        logger.error(f"Failed to fetch OpenAPI spec: {response.status}")
                        return documents
                    # GitHub serves raw files as text/plain, so we must tell aiohttp to parse as JSON anyway
                    spec = await response.json(content_type=None)
        ...
        for path, operations in paths.items():
            for method, op_data in operations.items():
                # Skip parameters or other non-method fields at the path level
                if method.lower() not in ("get", "post", "put", "delete", "patch"):
                    continue

                title = op_data.get("summary", f"{method.upper()} {path}")
                description = op_data.get("description", "")
                
                # Build a markdown representation of the endpoint
                md_lines = [
                    f"# {title}",
                    "",
                    description,
                ...
                # Optional: Add parameter info if available
                ...
                if parameters:
                    ...
                    for param in parameters:
                        if "name" in param:
                            ...
                            md_lines.append(f"- **{param['name']}**{req}: {desc}")

                content = "\n".join(md_lines)
                doc_url = f"https://docs.stripe.com/api/{path.replace('/', '_').strip('_')}"
        ...
        logger.info(f"Parsed {len(documents)} API endpoints from OpenAPI spec")
        return documents
    ...
    async def scrape_stripe_changelog(
        ...
        """
        Scrape Stripe changelog entries from the stripe/openapi GitHub releases.
        ...
        """
        url = "https://api.github.com/repos/stripe/openapi/releases"
        documents: list[RawDocument] = []

        logger.info(f"Fetching Stripe changelog from: {url}")
        ...
        try:
            async with aiohttp.ClientSession(headers=headers) as session:
                async with session.get(f"{url}?per_page={min(100, max_entries)}", timeout=self.STRIPE_TIMEOUT_SECONDS) as response:
                    if response.status != 200:
                        logger.error(f"Failed to fetch changelog releases: {response.status}")
                        return documents
        ...
        for release in releases[:max_entries]:
            title = release.get("name") or release.get("tag_name", "Release")
            content = release.get("body", "")
            date_str = release.get("published_at", "")
            
            # Format to match the previous changelog parser expectation (starts with ## Date)
            date_formatted = date_str.split("T")[0] if date_str else "Unknown Date"
            formatted_content = f"## {date_formatted}\n\n{content}"
        ...
        logger.info(f"Parsed {len(documents)} changelog entries from GitHub releases")
        return documents
